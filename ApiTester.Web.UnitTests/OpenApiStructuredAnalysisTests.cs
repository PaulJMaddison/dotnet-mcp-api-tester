...
public sealed class OpenApiStructuredAnalysisTests
{
    ...
    public void ConstraintGenerator_ResolvesReferencedRequestBodySchema()
    {
        ...
        Assert.DoesNotContain(plan.TestCases, c => c.Category == "optional" && c.Target is "requestBody.amount" or "requestBody.currency");
    }
    ...
    [Fact]
    public void OperationInventory_ListsContractOperationsDeterministically()
    {
        ...
        var inventory = new DescribeTools(store).ApiListOperations();
        var json = System.Text.Json.JsonSerializer.Serialize(inventory);

        Assert.Contains("createPayment", json);
        Assert.Contains("POST", json);
        Assert.Contains("/customers/{customerId}/payments", json);
    }
