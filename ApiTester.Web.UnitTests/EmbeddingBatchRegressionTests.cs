...
{
    [Fact]
    public async Task AzureEmbeddingClient_BatchesMultipleInputsIntoOneRequest()
    {
    ...
    {
        var embeddings = new FailingBatchEmbeddingClient();
        var store = new RecordingVectorStore();
        var indexer = new RagIndexer(embeddings, store);
    ...
    private sealed class EmbeddingHandler : HttpMessageHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            LastInputCount = input.GetArrayLength();

            var data = Enumerable.Range(0, LastInputCount)
                .Reverse()
                .Select(index => new { index, embedding = new[] { (float)(index + 1), 0.5f } })
                .ToArray();
    ...
    {
        public Task<float[]> EmbedAsync(string text, CancellationToken ct)
            => throw new InvalidOperationException("single path should not be used");

        public Task<IReadOnlyList<float[]>> EmbedBatchAsync(IReadOnlyList<string> texts, CancellationToken ct)
            => throw new InvalidOperationException("provider failed before publication");
    }
