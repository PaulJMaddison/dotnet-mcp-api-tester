...
public sealed class PublicApiRegressionTests
{
    ...
    public void SynthesisedOperationId_IsConsumableByDiscoveryAndGenerator()
    {
        var document = Parse("""
        {
          ...
          "paths":{"/get":{"get":{"responses":{"200":{"description":"OK"}}}}}
        }
        """);
    ...
    [Fact]
    public void ProjectBoundStore_FailsClosedAfterProjectSwitch()
    {
        var context = new ProjectContext();
        var projectA = Guid.NewGuid();
        var projectB = Guid.NewGuid();
        context.SetCurrentProject(projectA);

        var store = new OpenApiStore(context);
        store.SetDocument(projectA, Parse(SimpleSpec("API A")), "a.json");
        Assert.True(store.HasDocument);
        Assert.Equal(projectA, store.ProjectId);

        context.SetCurrentProject(projectB);

        Assert.False(store.HasDocument);
        Assert.Null(store.Document);
        var ex = Assert.Throws<InvalidOperationException>(store.RequireDocument);
        Assert.Contains("current project", ex.Message, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void ConstraintGenerator_UsesInt64Extremes()
    {
        ...
        {
          "openapi":"3.0.1",
          "info":{"title":"Int64","version":"1"},
          "paths":{"/pets/{petId}":{"get":{"operationId":"getPet","parameters":[{"name":"petId","in":"path","required":true,"schema":{"type":"integer","format":"int64"}}],"responses":{"200":{"description":"OK"}}}}}
        }
        """);

        var plan = new OpenApiConstraintTestGenerator().Generate(document, "getPet");
        var values = plan.TestCases
            .Where(c => c.Category == "numeric-extreme")
            .Select(c => c.Inputs["petId"])
            .ToArray();
    ...
    public void ConstraintGenerator_TreatsObjectPropertyAsObjectNotString()
    {
        var document = Parse("""
        {
          ...
          "paths":{"/pets":{"post":{"operationId":"addPet","requestBody":{"required":true,"content":{"application/json":{"schema":{"$ref":"#/components/schemas/Pet"}}}},"responses":{"200":{"description":"OK"}}}}},
          "components":{"schemas":{"Pet":{"type":"object","properties":{"category":{"$ref":"#/components/schemas/Category"}}},"Category":{"type":"object","required":["id"],"properties":{"id":{"type":"integer","format":"int64"},"name":{"type":"string"}}}}}
        }
        """);
    ...
    [Fact]
    public void ImportLimit_AcceptsCurrentGitHubBundledContractSizeWithHeadroom()
    {
    ...
    [Fact]
    public void AzureCredentialSource_RejectsUnknownSource()
    {
        var options = new AzureOpenAiOptions { CredentialSource = "WhateverWasCached" };
        Assert.Throws<InvalidOperationException>(options.GetCredentialSource);
    }
    ...
    private static string SimpleSpec(string title) => $$"""
    {
      "openapi":"3.0.1",
