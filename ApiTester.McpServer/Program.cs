...
builder.Services.AddSingleton<IEmbeddingClient>(sp =>
{
    ...
    {
        logger.LogInformation("Using Azure OpenAI embedding deployment {EmbeddingDeployment} for RAG retrieval.", options.EmbeddingDeployment);
        return new AzureOpenAiEmbeddingClient(sp.GetRequiredService<AzureOpenAiTransport>(), options);
    }
...
builder.Services.AddSingleton<IAiClient>(sp =>
{
    ...
    {
        logger.LogInformation("Using Azure OpenAI chat deployment {ChatDeployment} for grounded answers.", options.ChatDeployment);
        return new AzureOpenAiClient(sp.GetRequiredService<AzureOpenAiTransport>(), options);
    }
