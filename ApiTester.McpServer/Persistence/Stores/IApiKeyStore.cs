...
public interface IApiKeyStore
{
    ...
    Task<IReadOnlyList<ApiKeyRecord>> ListByPrefixAsync(string prefix, CancellationToken ct);
    Task<ApiKeyRecord?> RevokeAsync(Guid organisationId, Guid keyId, DateTime revokedUtc, CancellationToken ct);
    Task<ApiKeyRecord?> TouchLastUsedAsync(Guid keyId, DateTime lastUsedUtc, CancellationToken ct);
}
